...
#CPU = ["ThinkPadX120e"]
CPU = [
       # ("LatitudeE6400", "11-06_FFT"),
       # ("LatitudeE6500", "11-06_FFT"),
        ("Latitude7480", "Latitude7480")
        ]
...
def generate_plots(data, x_cat, y_cat, machines, test_name, pf = True, flags = None, save = True):
    ...
    for m in machines:
        for c in data[m[0]]:
            fs = (data[m[0]][c] if flags == None else flags)
            for f in fs:
                pfs = (data[m[0]][c][f] if pf else ["False"])
                for p in pfs:
                    # [:-1] --> exclude the multi from the data to plot
                    to_plot = data[m[0]][c][f][p][:-1]
    ...
    plt.title(y_cat + " vs. " + x_cat + \
              ((" for " + machines[0][0]) if one_m else ""))
...
def main():
    ...
    x_cat = 'cycles'
    y_cat = [
            'L1-dcache-loads', 'context-switches', 
            'L1-dcache-load-misses', 'context-switches', 
    ...
    for machine in CPU:
        data = generate_data(data, machine[0], machine[1])

    for cat in y_cat:
        generate_plots(data, x_cat, cat, CPU, "NULL")
